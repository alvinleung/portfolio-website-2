...
var sketch = function (p) {
    ...
    p.setup = function () {
        p.createCanvas(document.body.clientWidth, document.body.clientHeight, p.WEBGL);
        p.rectMode(p.CENTER);
    };
    p.draw = function () {
        p.clear();
    ...
    function getRandomScreenPos() {
        return p.createVector(Math.random() * p.width, Math.random() * p.height);
    }
    ...
    function updateWormLogic(worm) {
        ...
        if (!p.mouseIsPressed) {
            ...
            var attractiveness = DELAY_FACTOR * (attractivenessByWormGene * geneVariationInfluence + 1);
            worm.vel.set(mouseWormDistVec.add(mouseMovementProjectionOffset).mult(attractiveness));
        } else {
            if (!randomLocation[worm.index]) {
                var vec = getRandomScreenPos();
                randomLocation[worm.index] = vec;
            }
            worm.vel.set(randomLocation[worm.index].copy().sub(worm.pos).mult(0.1));
        }
        updateWormOrbit(worm);
        updateWormJoints(worm);
    }
    function updateWormOrbit(worm) {
        worm.pos.add((p.noise(worm.pos.x, worm.pos.y) - 0.46) * 50, (p.noise(worm.pos.y, worm.pos.x) - 0.46) * 50);
    }
...
$fsx.f[1] =
function(){
$fsx.r('fuse-box-css')('default/style.css', '* {\n  margin: 0rem;\n  box-sizing: border-box;\n}\n\nhtml,\ncanvas,\nbody {\n  display: block;\n  /* width: 100%;\n    height: 100vh; */\n  position: absolute;\n  bottom: 0rem;\n  top: 0rem;\n  left: 0rem;\n  right: 0rem;\n}\n');
}
$fsx.r(0)
})($fsx);
